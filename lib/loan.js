...
export function homeCosts({ amount, downPayment = 0, apr, years, propertyTax, homeInsurance }) {
  ...
  // Round each part, then total the rounded parts, so the breakdown shown to
  // the user always adds up to the true monthly figure (no off-by-a-dollar).
  const pi = Math.round(M);
  const tax = Math.round(taxAnnual / 12);
  const ins = Math.round(insAnnual / 12);
  const pmi = Math.round(pmiAnnual / 12);

  return {
    principal_interest_monthly: pi,
    property_tax_monthly: tax,
    home_insurance_monthly: ins,
    pmi_monthly: pmi,
    true_monthly_piti: pi + tax + ins + pmi,
    has_pmi: hasPmi,
